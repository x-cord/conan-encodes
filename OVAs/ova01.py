...
def effortfm(conan):
    ...
    fm2 = awf.ReplaceFrames(fm1, core.std.MaskedMerge(fm1, qtgmc, core.std.MaskedMerge(fm.std.BlankClip().std.ShufflePlanes(planes=0, colorfamily=GRAY), fm1.comb.CombMask(cthresh=2, mthresh=10, mi=30).std.ShufflePlanes(planes=0, colorfamily=GRAY).std.BoxBlur(hradius=8).std.Maximum().std.Maximum().std.Maximum().std.Maximum().std.Maximum().std.Binarize(35).std.BoxBlur(hradius=8), kgf.squaremask(fm, width=332, height=226, offset_x=330, offset_y=110), first_plane=True)), "39997 40069 40119 40137 40469 40531 40648 40687 40906 41173 41465 41508 41614 41618 41629 41632 41702 41806 41835 41963 42000")
    fm = awf.ReplaceFrames(fm2, core.std.MaskedMerge(fm2, fm, kgf.squaremask(fm, width=332, height=226, offset_x=330, offset_y=110), first_plane=True), "[40156 40344] [41136 41172] [41174 41390] [41534 41631] [41633 41701] [41703 41805] [41807 41834] [41836 41962] [41964 41999]")
